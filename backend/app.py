...
def init_pg_schema():
    ...
    try:
        ...
        # Determine proposals.id type (integer vs uuid) so collaborators FK matches existing schema
        try:
            cursor.execute("""
                SELECT data_type
                FROM information_schema.columns
                WHERE table_name = 'proposals' AND column_name = 'id'
            """)
            _collab_proposals_id_type_row = cursor.fetchone()
            _collab_proposals_id_type = (
                _collab_proposals_id_type_row[0]
                if _collab_proposals_id_type_row and _collab_proposals_id_type_row[0]
                else 'integer'
            )
        except Exception:
            # Fallback for older databases where proposals.id is INTEGER
            _collab_proposals_id_type = 'integer'

        # Active collaborators table (tracks collaborators who have accessed the proposal)
        if _collab_proposals_id_type == 'uuid':
