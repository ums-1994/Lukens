...
# Short-circuit OPTIONS preflight at the ASGI layer so that
# preflight requests always get an HTTP 200 with CORS headers
# even if the Flask routing does not find a matching route.
# This avoids browser preflight failures when uvicorn is serving
# the ASGI app directly.
_original_asgi_app = asgi_app

async def _asgi_preflight_handler(scope, receive, send):
    try:
        if scope.get('type') == 'http' and scope.get('method') == 'OPTIONS':
            ...
            if origin:
                headers.extend([
                    ...
                    (b'access-control-allow-headers', b'Content-Type, Authorization, Collab-Token'),
            ...
            await send({
                ...
                'body': b'{"ok": true, "preflight": true}',
                'more_body': False
            })
            return
    except Exception:
        # On any error, fall through to the original app
        pass

    await _original_asgi_app(scope, receive, send)

asgi_app = _asgi_preflight_handler
...
def get_pg_pool():
    ...
    if _pg_pool is None:
        ...
        # Retry loop to handle cases where Postgres is still starting (Docker healthcheck)
        ...
        while attempt < max_retries:
            attempt += 1
            try:
                print(f"🔄 Connecting to PostgreSQL: {db_config['host']}:{db_config['port']}/{db_config['database']} (attempt {attempt}/{max_retries})")
                ...
                if attempt >= max_retries:
                    print("❌ Exceeded maximum DB connection retries")
                    break
                time.sleep(delay)
                # exponential backoff with cap
                delay = min(delay * 1.5, 30)

        if last_err is not None and _pg_pool is None:
            print(f"❌ Error creating PostgreSQL connection pool after {max_retries} attempts: {last_err}")
            raise last_err
...
def handle_cors_after_request(response):
    ...
    # Debug: log origin and whether CORS headers are present
    try:
        print(f"[CORS] request Origin: {origin}, existing CORS headers: {', '.join(k for k in response.headers.keys() if k.lower().startswith('access-control-'))}")
    except Exception:
        pass
    ...
    # Allow any localhost dev origin (useful for Flutter/dev servers which assign dynamic ports)
    if origin and (
        origin.startswith('http://localhost') or origin.startswith('http://127.0.0.1') or origin.startswith('http://[::1]')
    ):
        if 'Access-Control-Allow-Origin' not in response.headers:
            response.headers['Access-Control-Allow-Origin'] = origin
            response.headers['Access-Control-Allow-Credentials'] = 'true'
    ...
    # Debug: final CORS headers after possible modification
    try:
        print(f"[CORS] response headers now include: {', '.join(k for k in response.headers.keys() if k.lower().startswith('access-control-'))}")
    except Exception:
        pass
...
def init_db():
    ...
    # If this is a CORS preflight (OPTIONS) request, respond immediately with
    # a minimal 200 OK and CORS headers so the browser's preflight succeeds.
    # This avoids the browser blocking the subsequent request when a route
    # returns a non-2xx status for OPTIONS.
    if request.method == 'OPTIONS':
        ...
        if origin:
            ...
            resp.headers['Access-Control-Allow-Headers'] = 'Content-Type, Authorization, Collab-Token'
...
# Debug endpoint to verify CORS behavior quickly during local development
@app.route('/_debug_cors', methods=['OPTIONS', 'GET', 'POST'])
def _debug_cors():
    origin = request.headers.get('Origin') or ''
    print(f"[_debug_cors] received request. Origin={origin}, Method={request.method}")
    from flask import Response, jsonify
    resp = jsonify({'ok': True, 'origin': origin, 'method': request.method})
    if origin:
        resp.headers['Access-Control-Allow-Origin'] = origin
        resp.headers['Access-Control-Allow-Credentials'] = 'true'
        resp.headers['Access-Control-Allow-Headers'] = 'Content-Type, Authorization, Collab-Token'
