...
Usage (from repo root or backend folder):
  PowerShell> .\backend\scripts\apply_firebase_uid_index.ps1
  or
  PowerShell> .\backend\scripts\apply_firebase_uid_index.ps1 -Container unathi-postgres -MigrationFile ..\migrations\001_create_unique_index_firebase_uid.sql
...
if ($LASTEXITCODE -ne 0 -or -not $envDump) {
    FailExit "Failed to inspect container '$Container'. Output:\n$envDump"
}
...
if ($dups -match "ERROR|FATAL") {
    FailExit "psql error while checking duplicates:\n$dups"
}

$dupsTrim = $dups.Trim()
if ($dupsTrim -eq "") {
    Write-Output "No duplicates found. Applying unique index from migration file: $MigrationFile"
