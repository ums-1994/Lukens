        ...
        with get_db_connection() as conn:
            ...
            if user_id:
                ...
                if auto_created:
                    ...
                    print(
                        f"✅ User {user_id} was auto-created in this request "
                        "(auto_created=True), skipping DB verification"
                    )
                else:
                    # Backward-compatible fallback: some flows may store this on flask.g.
                    try:
                        from flask import g

                        auto_created_ctx = getattr(g, '_auto_created_user', None)
                        if auto_created_ctx and auto_created_ctx.get('user_id') == user_id:
                            auto_created_in_request = True
                            print(
                                f"✅ User {user_id} was auto-created in this request "
                                "(flask.g), skipping DB verification"
                            )
