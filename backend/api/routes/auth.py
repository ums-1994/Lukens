...
# --- CORS Preflight for /firebase and /api/auth/firebase ---
...
def _cors_preflight_response():
    ...
    if origin:
        ...
        response.headers.add('Access-Control-Allow-Headers', 'Content-Type, Authorization, Collab-Token')
        response.headers.add('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, PATCH, OPTIONS')
    return response, 204  # Always return 204 for OPTIONS

@bp.route("/firebase", methods=['OPTIONS'])
def options_firebase_auth():
    """Handle CORS preflight for Firebase auth endpoints"""
...
def firebase_auth():
    ...
    try:
        # ...existing firebase POST logic (unchanged, see above)...
        data = request.get_json(silent=True)
        if data is None:
            raw = request.get_data(as_text=True)
            content_type = request.content_type
            print(f'Firebase auth: content_type={content_type}, raw_length={len(raw) if raw is not None else 0}')
            if raw:
                try:
                    data = json.loads(raw)
                except Exception as e:
                    print(f'Firebase auth JSON decode error on raw body: {e}')
                    if request.form and len(request.form) > 0:
                        data = request.form.to_dict()
                        print('Firebase auth: using form-data fallback')
                    else:
                        if os.getenv('DEV_DEBUG_RAW', 'false').lower() in ['1', 'true', 'yes']:
                            snippet = raw[:200] if raw else ''
                            return {'detail': 'Invalid JSON or malformed request body', 'raw_snippet': snippet}, 400
                        return {'detail': 'Invalid JSON or malformed request body'}, 400
            else:
                return {'detail': 'Invalid JSON or missing Content-Type header or empty body'}, 400
...
@bp.post("/link/firebase")
@token_required
def link_firebase_account(username=None):
    """Link a Firebase account to the current authenticated user"""
    try:
        ...
        if not fb_user:
            return {'detail': 'Could not extract Firebase user info'}, 400

        fb_uid = fb_user.get('uid')
        fb_email = (fb_user.get('email') or '').lower()

        if not fb_uid:
            return {'detail': 'Firebase UID missing'}, 400

        conn = _pg_conn()
        cursor = conn.cursor()
        try:
            # Verify current user exists and fetch their email
            cursor.execute('SELECT id, email FROM users WHERE username = %s', (username,))
            row = cursor.fetchone()
            if not row:
                return {'detail': 'Current user not found'}, 404
            user_id, user_email = row[0], (row[1] or '').lower()

            # Require email match or firebase verified email to avoid takeover
            fb_email_verified = fb_user.get('email_verified', False)
            if fb_email and (fb_email != user_email) and not fb_email_verified:
                return {'detail': 'Email mismatch — cannot link accounts without verified Firebase email'}, 403

            # Ensure firebase_uid isn't already linked to another user
            cursor.execute('SELECT id FROM users WHERE firebase_uid = %s AND id != %s', (fb_uid, user_id))
            if cursor.fetchone():
                return {'detail': 'This Firebase account is already linked to another user'}, 409

            # Update users firebase_uid
            try:
                cursor.execute('UPDATE users SET firebase_uid = %s WHERE id = %s', (fb_uid, user_id))
                conn.commit()
            except Exception as e:
                conn.rollback()
                print(f'Link Firebase DB error: {e}')
                return {'detail': 'Database error while linking Firebase account'}, 500

            return {'detail': 'Firebase account linked', 'firebase_uid': fb_uid}, 200
        finally:
            release_pg_conn(conn)
    except Exception as e:
        print(f'Link Firebase error: {e}')
        traceback.print_exc()
        return {'detail': str(e)}, 500


@bp.post("/unlink/firebase")
@token_required
def unlink_firebase_account(username=None):
    """Unlink Firebase from the authenticated user"""
    try:
        conn = _pg_conn()
        cursor = conn.cursor()
        try:
            ...
            user_id = row[0]
            try:
                cursor.execute('UPDATE users SET firebase_uid = NULL WHERE id = %s', (user_id,))
                conn.commit()
            except Exception as e:
                conn.rollback()
                print(f'Unlink Firebase DB error: {e}')
                return {'detail': 'Database error while unlinking Firebase account'}, 500

            return {'detail': 'Firebase account unlinked'}, 200
        finally:
            release_pg_conn(conn)
    except Exception as e:
        print(f'Unlink Firebase error: {e}')
        traceback.print_exc()
        return {'detail': str(e)}, 500
