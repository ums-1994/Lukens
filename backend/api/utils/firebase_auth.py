    ...
    try:
        ...
        if not cred_path:
            ...
            if os.path.exists(default_path):
                ...
                print(f"[INFO] Using default Firebase credentials: {default_path}")
        ...
        if cred_path and os.path.exists(cred_path):
            ...
            if service_account_json:
                ...
                try:
                    ...
                    print(f"[WARN] Firebase Admin SDK not initialized: {e}")
...
def verify_firebase_token(id_token):
    ...
    try:
        ...
        if _firebase_app is None:
            print("[WARN] Firebase not initialized, cannot verify token")
            return None
        ...
        if not id_token or len(id_token.split('.')) != 3:
            print(f"[WARN] Token doesn't look like a Firebase ID token (JWT format required)")
            return None
        
        # Verify the ID token
        decoded_token = auth.verify_id_token(id_token)
        print(f"[OK] Firebase ID token verified successfully")
        return decoded_token
    except auth.InvalidIdTokenError as e:
        print(f"[ERROR] Invalid Firebase ID token: {str(e)}")
        return None
    except auth.ExpiredIdTokenError as e:
        print(f"[ERROR] Expired Firebase ID token: {str(e)}")
        return None
    except ValueError as e:
        # Token format errors
        print(f"[WARN] Token format error (likely not a Firebase token): {str(e)}")
        return None
    except Exception as e:
        print(f"[ERROR] Error verifying Firebase token: {type(e).__name__}: {str(e)}")
        return None
