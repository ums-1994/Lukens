  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          const Text("Content Library", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(child: TextField(controller: keyCtrl, decoration: const InputDecoration(labelText: "Key (unique)"))),
              const SizedBox(width: 8),
              Expanded(child: TextField(controller: labelCtrl, decoration: const InputDecoration(labelText: "Label"))),
              const SizedBox(width: 8),
              ElevatedButton(onPressed: () async {
                ...
                try {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Failed to create content: $e")));
                }
            ...
            child: ListView.builder(
              ...
              itemBuilder: (ctx, i) {
                ...
                return ListTile(
                  ...
                  subtitle: Text((b["content"] ?? "").toString(), maxLines: 2, overflow: TextOverflow.ellipsis),
                  trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                    IconButton(icon: const Icon(Icons.add_box_outlined), onPressed: () async {
                      if (app.currentProposal == null) {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Select a proposal first")));
                        return;
                      }
                      await app.addContentToProposal(b["id"]);
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Added to proposal")));
                    }),
