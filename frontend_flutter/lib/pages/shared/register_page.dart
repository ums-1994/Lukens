...
class _RegisterPageState extends State<RegisterPage> with TickerProviderStateMixin {
  // Social login methods (copied and adapted from LoginPage)
    ...
    try {
      ...
      if (credential.user != null) {
        ...
        if (firebaseIdToken != null && firebaseIdToken.isNotEmpty) {
          print('Google login success, token: \\${firebaseIdToken.substring(0, 20)}...');
          // Sync with backend
          await _handleSocialLogin(firebaseIdToken);
        } else {
          print('Google login failed: No token received.');
        }
      }
    } catch (e) {
  ...
  Future<void> _signInWithMicrosoft() async {
    ...
    try {
      ...
      if (credential.user != null) {
        final firebaseIdToken = await credential.user!.getIdToken();
        if (firebaseIdToken != null && firebaseIdToken.isNotEmpty) {
          print('Microsoft login success, token: \\${firebaseIdToken.substring(0, 20)}...');
          // Sync with backend
          await _handleSocialLogin(firebaseIdToken);
        } else {
          print('Microsoft login failed: No token received.');
        }
      }
    } catch (e) {
      print('Microsoft login error: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Microsoft login failed: $e'), backgroundColor: Colors.red),
      );
    } finally {
      if (mounted) setState(() => _isLoading = false);
    }
  ...
  // Common handler for social login backend sync and navigation
  Future<void> _handleSocialLogin(String firebaseIdToken) async {
    try {
      // Send Firebase ID token to backend to create/update user in database
      print('📡 Syncing social user to backend database...');
      final response = await http.post(
          Uri.parse('${AuthService.baseUrl}/api/auth/firebase'),
      ...
      if (userProfile != null) {
        ...
        if (userRole == 'admin' || userRole == 'ceo' || _selectedRole.toLowerCase() == 'admin') {
      ...
      print('❌ Social registration error: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        ...
        onPressed: _isLoading
            ...
            : () {
                if (icon == Icons.g_mobiledata) {
                  _signInWithGoogle();
                } else if (icon == Icons.window || icon == Icons.business) {
