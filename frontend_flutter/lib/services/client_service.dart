...
class ClientService {
  ...
  static Future<List<dynamic>> getClientsForSelection(String token, {String? search}) async {
    try {
      final uri = search != null && search.isNotEmpty
          ? Uri.parse('$baseUrl/api/clients/for-selection?search=$search')
          : Uri.parse('$baseUrl/api/clients/for-selection');
      ...
      print('Error fetching clients for selection: ${response.statusCode} - ${response.body}');
      return [];
    } catch (e) {
      print('Error fetching clients for selection: $e');
      return [];
    }
  }
